...
//！！！！！！！！！！！！！
//显式实例化模板模板，但是该特性不支持使用，编程过程中最好全部放入头文件中
#define _EXPLICIT_DEFINE(CT) template class algorithmLL::Strassen_class<CT>
...
template<typename T>
void algorithmLL::Strassen_class<T>::ADD(T **MatrixA, T **MatrixB, T **MatrixResult, int MatrixSize)
{
...
template<typename T>
void algorithmLL::Strassen_class<T>::SUB(T** MatrixA, T** MatrixB, T** MatrixResult, int MatrixSize )
{
    for ( int i = 0; i < MatrixSize; i++)
    {
        for ( int j = 0; j < MatrixSize; j++)
...
template<typename T>
void algorithmLL::Strassen_class<T>::MUL( T** MatrixA, T** MatrixB, T** MatrixResult, int MatrixSize )
{
    for (int i=0;i<MatrixSize ;i++)
    {
        for (int j=0;j<MatrixSize ;j++)
...
*/
template<typename T>
void algorithmLL::Strassen_class<T>::Strassen(int N, T **MatrixA, T **MatrixB, T **MatrixC)
{
...
template<typename T>
void algorithmLL::Strassen_class<T>::FillMatrix( T** MatrixA, T** MatrixB, int length)
{
...
template<typename T>
void algorithmLL::Strassen_class<T>::PrintMatrix(T **MatrixA,int MatrixSize)
{
    cout<<endl;
    for(int row = 0; row<MatrixSize; row++)
    {
        for(int column = 0; column<MatrixSize; column++)
        {
            ...
            if ((column+1)%((MatrixSize)) == 0)//在内循环的最后一次输出换行
                cout << endl;
        }
    }
    cout<<endl;
